...
public class UserRegistrationControllerPortlet extends MVCPortlet {
	...
	public void userRegistration(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		try {
			...
			Group group = GroupLocalServiceUtil.getGroup(themeDisplay.getCompanyId(), UserRegistrationControllerPortletKeys.SITE_NAME_CROWD_FUNDING);
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
			throws IOException, PortletException {
		try {
			String serverResourceParameter = ParamUtil.getString(resourceRequest, "serverResourceParameter");
			long mobileNo = ParamUtil.getLong(resourceRequest, "mobileNoNirav");
			
			if(serverResourceParameter.equalsIgnoreCase("OtpExpire")) {
			...
			if(serverResourceParameter.equalsIgnoreCase("ResendOtp")) {
				System.out.println("Nirav:::" + mobileNo);
				UserOTPRegistration otpObj = UserOTPRegistrationLocalServiceUtil.findByMOBILE_NO(mobileNo);
				Random rnd = new Random();
			    int otpNumber = rnd.nextInt(999999);
