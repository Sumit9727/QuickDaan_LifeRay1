...
<portlet:resourceURL var = "serveResourceOtpExpire" >
	<portlet:param name="serverResourceParameter" value="OtpExpire" />
	<portlet:param name="mobileNoNirav" value="<%= String.valueOf(mobileNo)  %>" />
</portlet:resourceURL>

<portlet:resourceURL var = "serveResourceResendOtp" >
	<portlet:param name="serverResourceParameter" value="ResendOtp" />
	<portlet:param name="mobileNoNirav" value="<%= String.valueOf(mobileNo)  %>" />
</portlet:resourceURL>
        ...
        if((current_minutes ==0) && (seconds == 0)){
        	...
        	window.location = "http://localhost:8080/web/crowd-funding/login";
	        	...
	        	$(document).ready(function() {
	    			...
	    			$.ajax({
	    			dataType: 'json',
	    		    url:'${serveResourceOtpExpire.toString()}',
...
function myStopFunction() {
	$(document).ready(function() {
		...
		$.ajax({
		dataType: 'json',
	    url:'${reSendOtpRegistrationURL.toString()}',
	    data: {'<portlet:namespace/>mobileNo':mobileNoValue},                
	    success: function(response){
	        console.log(response);
