...
public class UserRegistrationControllerPortlet extends MVCPortlet {
	...
	public void emailMobileRegistration(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		try {
			...
			try {
				UserOTPRegistration otpObj = UserOTPRegistrationLocalServiceUtil.findByMAIL_OTP_STATUS(email, UserRegistrationControllerPortletKeys.OTP_VALUE_1);
				
				UserRegistration userObj = UserRegistrationLocalServiceUtil.findByUSER_OTP_ID(otpObj.getUSER_OTP_ID());
	...
	public void userRegistration(ActionRequest actionRequest, ActionResponse actionResponse)
		...
		try {
			...
			//HPMCConstant.SITE_NAME=site;
			Group group = GroupLocalServiceUtil.getGroup(themeDisplay.getCompanyId(),UserRegistrationControllerPortletKeys.CrowdFunding_Site);
			UserUtil.addGroup(farmerUser.getUserId(), group);
