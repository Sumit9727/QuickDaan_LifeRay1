		...
		<thead>
			...
			<portlet:renderURL var="PositionRegistrationUrl" windowState="normal">
				<portlet:param name="PositionId" value="<%= String.valueOf(PositionObj.getPOSITION_ID()) %>" />
			 	<portlet:param name="jspPage" value="/jsp/currentcarrier/ActivePostDetailPage.jsp"/>
			  </portlet:renderURL>
			  <portlet:renderURL var="EditPositionRegistrationUrl" windowState="normal">
				<portlet:param name="PositionId" value="<%= String.valueOf(PositionObj.getPOSITION_ID()) %>" />
			 	<portlet:param name="jspPage" value="/jsp/currentcarrier/EditPostedForm.jsp"/>
			  </portlet:renderURL>
			  <portlet:actionURL name="changeActiveToInactive" var="changeActiveToInactiveURL">
					<portlet:param name="PositionId" value="<%= String.valueOf(PositionObj.getPOSITION_ID()) %>" />
				</portlet:actionURL>
				<td><%= PositionObj.getPOSITION_ID() %></td>
				 ...
				 <liferay-ui:icon-menu>
					 	<liferay-ui:icon image="view" message="Details" url="${PositionRegistrationUrl}" />
						 		<liferay-ui:icon image="edit" message="Edit Position" url="${EditPositionRegistrationUrl}" />
								<liferay-ui:icon image="edit" message="Inactive Job" url="${changeActiveToInactiveURL}"/>
				    </liferay-ui:icon-menu>
				 </td>
				 </tr>
			<%
